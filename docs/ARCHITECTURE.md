...
Seed the graph and program overlays from official curriculum material before learner data exists (ADR-0013). Extraction and LLM-assisted decomposition supply draft concepts, ladders, and inferred prerequisites; retain objective identifiers, source/version/location, and review status so imported requirements remain distinguishable from inferred structure. The interfaces below are a conceptual sketch; provenance fields remain to be specified before implementing import.

Placement combines school year/program (age as a rough prior), optional images of recent teacher assignments, and targeted diagnostic attempts. Classroom topic, curriculum expectation, and demonstrated learner understanding remain separate. Checked exercise/explanation generation uses the selected objective and learner evidence; imported graph coverage may exceed validated tutoring coverage. The workflow and official sources are in `BOOTSTRAP-PILOT.md`.
...
Historical first-cut magnitudes, interpreted **per ladder rung** and derived from encounter history (ADR-0018). **ADR-0021 makes these values and their ordering provisional:** they are not calibrated probabilities, mandatory discovery bonuses, or evidence of efficacy. The table remains a candidate baseline for evaluation, not a rule to report as learner truth.
...
The earlier exam-mode multiplier of 0.7 and the discovered-slower-than-delivered decay rule are hypotheses to compare with alternatives, including no mode discount and equal decay (ADR-0021). Record actual support and demonstrated reasoning; mode or teaching route alone does not establish stronger or weaker understanding. The expressive-baseline adjustment (ADR-0012) also needs validation against independent outcomes.

The structural commitment is honest attribution of independent demonstration versus supplied content (amended F2), with evidence weight and uncertainty preserved. The per-rung, history-derived model remains; teaching-route weights and decay must be calibrated using delayed independent evidence (ADRs 0018 and 0021).
...
**Network access and incentives are deferred.** Contributor-only benchmarks, contribution discounts, and isolation premiums remain ideas for a future business design. They are not pilot permissions or initial pricing rules. No institution must contribute data to buy the first product. ADR-0021 requires separate demand, validity, privacy, and governance work before a network is reconsidered.

Future aggregation infrastructure might be shared across AYA plugins if justified. That possibility neither establishes a moat nor creates a dependency for the Limpide pilot.

### Phasing

Phase 1 (pilot/MVP): retain the selected host and its data isolation. Implement learner history and the minimum explicitly authorized educator access needed for the pilot. Cohort reports are limited to useful, appropriately protected views; network computation, sharing, and benchmarks are out of scope. Preserve inexpensive scope distinctions and stable concept identifiers.

Phase 2 (after learner and buyer validation): develop useful formative cohort/institutional views with appropriate permissions and evidence standards. Network features remain deferred.

Future option, with no committed phase date: reconsider a network only when paid institutional value, comparable independently validated measures, sufficient willing and eligible institutions, and privacy/governance arrangements justify it. Network infrastructure and pricing need a new implementation decision at that point (ADR-0021).

Preserve privacy boundaries from the start, but do not require a complete future network design or a promise of zero schema changes before the pilot. Future migration work is acceptable when supported by real demand; speculative aggregation work must not delay validation.
